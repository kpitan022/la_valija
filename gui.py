...
def recorte():
    global img
    filename = tk.filedialog.askopenfilename(title = "Seleccione la imagen a recortar", filetypes = (("all files","*.*"),("jpeg files","*.jpeg"),("jpg files","*.jpg"),("png files","*.png"),("bmp files","*.bmp")))
...
def ruta_descarga():
    filename = tk.filedialog.askdirectory(title = "Seleccione la ruta de descarga ",)
    carpeta.set(filename)
    

def nueva_credencial(nombre, domicilio, numero_socio, dni,anio):
    if len(nombre.get()) == 0 or len(domicilio.get()) == 0 or len(numero_socio.get()) == 0 or len(dni.get()) == 0: 
    ...
    #Escribir texto
    cv2.putText(img, dni.get(), ubicacion_dni, font, tamañoLetra, colorLetra, grosorLetra)
    cv2.putText(img, nombre.get().title(), ubicacion_nombre, font, tamañoLetra, colorLetra, grosorLetra)
    cv2.putText(img, domicilio.get().title(), ubicacion_domicilio, font, tamañoLetra, colorLetra, grosorLetra)
    cv2.putText(img, numero_socio.get(), ubicacion_numero_socio, font, tamañoLetra, colorLetra, grosorLetra)
    cv2.putText(img, anio.get(), ubicacion_anio, font, tamañoLetra, colorLetra, grosorLetra)
    # preguntar si la imagen existe

    if len(carpeta.get()) != 0:
        carpeta_guardado = pathlib.Path(carpeta.get(), nombre.get().title() + ".jpeg")
        # comprueba si la credencial ya existe y si existe la borra
        if carpeta_guardado.exists():
            remove(str(carpeta_guardado))
        #Guardar imagen 
        cv2.imwrite(str(carpeta_guardado), img,)
        if 'recorte.jpg' in listdir('./'):
        ...
        dni.set("")
        anio.set("")
        messagebox.showinfo("Credencial", "Credencial guardada en " + str(carpeta_guardado))
...
# crear etiquetas
etiqueta_nombre = tk.Label(ventana, text=str("Nombre"))
etiqueta_domicilio = tk.Label(ventana, text="Domicilio:")
etiqueta_socio = tk.Label(ventana, text="N socio:")
etiqueta_dni = tk.Label(ventana, text="D.N.I:")
...
domicilio = tk.Entry(ventana, textvariable=domicilio_var)
socio = tk.Entry(ventana, textvariable=socio_var)
dni = tk.Entry(ventana, textvariable=dni_var)
anio = tk.Entry(ventana, textvariable=anio_var)
...
# boton filepath
button_explore = ttk.Button(ventana, text = "Ruta de descarga", command = ruta_descarga)
button_explore.grid(row=6, column=0,columnspan=2, sticky="ew")
# msg = ttk.Label(ventana,textvariable=carpeta)
